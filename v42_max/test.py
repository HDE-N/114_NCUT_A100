...
# =============================================================================
# 關鍵修正：適配新版 Global Max Pooling 骨架網路模型 
# =============================================================================
class GRUModel(nn.Module):
    ...
    def forward(self, x):
        ...
        # out_seq Shape: [batch_size, seq_len, hidden] -> 保留所有時間步特徵
        out_seq, _ = self.gru(x)
        
        # 核心修正：沿著時間軸 (dim=1) 擷取最大值，強制鎖定並帶出整段訊號中最強烈的疲勞特徵
        # torch.max 會回傳 (values, indices)，我們只需要數值 values
        feature, _ = torch.max(out_seq, dim=1) 
        
        # 安全修正：移除 .squeeze()。確保在邊緣端單筆推論 (batch_size=1) 時，
        # 輸出形狀依然穩定保持二維 [1, num_classes]，防止下游矩陣維度對齊出錯。
        out = self.fc(feature)
        return out, feature
...
def main():
    ...
    if len(calib_ds) == 0:
        ...
        # 1. 透過目前模型提取校正資料之 GRU 空間總體特徵 (此處已自動套用 Max Pooling)
