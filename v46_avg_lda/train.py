...
class GRUModel(nn.Module):
    ...
    def forward(self, x):
        ...
        # out_seq Shape: [batch_size, seq_len, hidden] -> 包含所有時間步的特徵
        out_seq, _ = self.gru(x)
        
        # 核心修改：沿著時間軸 (dim=1) 取平均值 (Global Average Pooling)
        # 融合整段 960 點的總體疲勞趨勢，直接回傳 Tensor，不需像 torch.max 一樣解包
        feature = torch.mean(out_seq, dim=1) 
        
        # 穩定輸出：保持二維 [batch_size, num_classes]，防止邊緣端單筆推論出錯
        out = self.fc(feature)
        return out, feature
