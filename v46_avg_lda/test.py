...
class TestDataset(Dataset):
    def __init__(self, folder, column_names, seq_len=960, only_notTired=False, calibration_mode=False):
        ...
        for p in files:
            try:
                if calibration_mode:
                    # 校正模式：不需真實標籤，預設給 0 且不跳過 any 檔案，用來完整估計 Target 領域骨架
                    y_label = 0
                elif only_notTired:
                    y_label = 0
                else:
...
# -----------------------------------------------------------------------------
# 核心優化點：將兩張獨立的 LDA 圖片合併成一個 1x2 的對比畫布
# -----------------------------------------------------------------------------
def plot_combined_lda_visualization(feats_base_2d, feats_aligned_2d, labels, target_names, save_path, xlim=None, ylim=None):
    """
    建立 1x2 子圖，將校正前與校正後的 LDA 投影特徵並列呈現，方便直接對比位移與旋轉效果。
    """
    try:
        # 建立 1列 2欄 的子圖畫布
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 7.5))
        color_map = {0: '#2ecc71', 1: '#e74c3c'} # 綠色與紅色
        
        # 1. 繪製左圖：未校正原始特徵 (Without CORAL Alignment)
        for c_idx in [0, 1]:
            if c_idx >= len(target_names): continue
            name = target_names[c_idx]
            mask = (labels == c_idx)
            if np.sum(mask) > 0:
                ax1.scatter(
                    feats_base_2d[mask, 0], feats_base_2d[mask, 1], 
                    label=name, color=color_map[c_idx], alpha=0.65, 
                    edgecolors='black', linewidths=0.6, s=45
                )
        ax1.set_xlabel('LD Component 1', fontsize=12, fontweight='bold')
        ax1.set_ylabel('LD Component 2', fontsize=12, fontweight='bold')
        ax1.set_title('(a) Before Alignment (Without CORAL)', fontsize=13, fontweight='bold', pad=10)
        ...
        # 2. 繪製右圖：校正對齊後特徵 (With CORAL Alignment)
        for c_idx in [0, 1]:
            if c_idx >= len(target_names): continue
            name = target_names[c_idx]
            mask = (labels == c_idx)
            if np.sum(mask) > 0:
                ax2.scatter(
                    feats_aligned_2d[mask, 0], feats_aligned_2d[mask, 1], 
                    label=name, color=color_map[c_idx], alpha=0.65, 
                    edgecolors='black', linewidths=0.6, s=45
                )
        ax2.set_xlabel('LD Component 1', fontsize=12, fontweight='bold')
        ax2.set_ylabel('LD Component 2', fontsize=12, fontweight='bold')
        ax2.set_title('(b) After Alignment (With CORAL)', fontsize=13, fontweight='bold', pad=10)
        ...
        # 全局大標題與排版調整
        plt.suptitle('GRU Spatial Feature Distribution Comparison via LDA (notTired vs Tired)', fontsize=15, fontweight='bold', y=0.98)
        plt.tight_layout()
        plt.savefig(save_path, dpi=300)
        plt.close()
        print(f"-> LDA 雙子對比合併圖已儲存：{save_path}")
    except Exception as e:
        print(f"[Warning] 產生合併 LDA 圖表失敗: {e}")
...
def main():
    ...
    with torch.no_grad():
        for segs, y, _ in loader:
            ...
            all_feats_base.append(feat_base.cpu().numpy())
            all_feats_aligned.append(feat_aligned.cpu().numpy())
    ...
    all_feats_aligned = np.concatenate(all_feats_aligned, axis=0)
    ...
    # --- 觀點 B 串接：LDA 特徵降維消融比較分析（合併成單張圖 + 固定全局軸刻度） ---
    lda_compare_path = os.path.join(output_dir, f"{test_run_name}_5_LDA_Comparison.png")
    
    print("\n[LDA] 正在建立並擬合原始特徵之 LDA 空間並計算 2D 投影軸...")
    # 1. 初始化 LDA 並完成對原始特徵的 fit
    lda_model = LinearDiscriminantAnalysis(n_components=2)
    feats_base_2d = lda_model.fit_transform(all_feats_base, all_y_true)
    
    # 2. 將對齊後的特徵直接映射至相同的空間（純 transform）
    feats_aligned_2d = lda_model.transform(all_feats_aligned)
    
    # 3. 計算兩組資料在 0(notTired) 與 1(Tired) 類別下的「全局一體化」邊界
    mask_01 = (all_y_true == 0) | (all_y_true == 1)
    if np.sum(mask_01) > 0:
        combined_x = np.concatenate([feats_base_2d[mask_01, 0], feats_aligned_2d[mask_01, 0]])
        combined_y = np.concatenate([feats_base_2d[mask_01, 1], feats_aligned_2d[mask_01, 1]])
        
        x_min, x_max = combined_x.min(), combined_x.max()
        y_min, y_max = combined_y.min(), combined_y.max()
        
        # 加上 10% Padding 讓圖像邊緣不被切到
    ...
    # 4. 繪製並輸出合併後的比較圖表，強制兩張子圖帶入完全一致的坐標軸範疇
    print("[LDA] 正在產出合併後的 LDA 比較圖表（固定全局軸刻度）...")
    plot_combined_lda_visualization(feats_base_2d, feats_aligned_2d, all_y_true, target_names, lda_compare_path, xlim=global_xlim, ylim=global_ylim)
    
    print(f"\n完成！所有圖片已儲存於 {output_dir}")
