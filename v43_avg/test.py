...
# =============================================================================
# 關鍵修正：適配新版 Global Average Pooling 骨架網路模型 
# =============================================================================
class GRUModel(nn.Module):
    ...
    def forward(self, x):
        ...
        # 核心修正：沿著時間軸 (dim=1) 取平均值 (Global Average Pooling)
        # 融合整段 960 點的總體疲勞趨勢，直接回傳 Tensor，不需進行解包
        feature = torch.mean(out_seq, dim=1) 
        
        # 穩定輸出：移除 .squeeze()，確保邊緣端單筆推論時輸出形狀依然穩定保持二維 [1, num_classes]
        out = self.fc(feature)
        return out, feature
...
def main():
    ...
    if len(calib_ds) == 0:
        ...
        # 1. 透過目前模型提取校正資料之 GRU 空間總體特徵 (此處已自動套用 Average Pooling)
