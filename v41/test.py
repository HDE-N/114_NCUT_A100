...
def main():
    ...
    coral_mu_src = stats['coral_mu_src'].to(device)         # 新版：全類別特徵中心
    coral_cov_src_sqrt = stats['coral_cov_src_sqrt'].to(device) # 新版：全類別特徵共變異數根號矩陣

    # =========================================================
    # 階段 2：邊緣端個人化校正 (全面移至全域骨架特徵與均值對齊)
    # =========================================================
    print("\n====== 正在執行邊緣端個人化校正 ======")
    calib_data_path = f"../data_v{args.target_version}"
    # 修改：開啟 calibration_mode=True 確保 5 分鐘資料完整載入計算總體特徵骨架
    ...
    if len(calib_ds) == 0:
        ...
        # 2. 計算 Base 版的動態閾值 (無特徵對齊，純出廠模型輸出結果)
        ...
        mu_tgt = calib_feat.mean(dim=0) # Target 總體中心
        tgt_centered = calib_feat - mu_tgt
        # 對角線加 1e-5 保障 SVD 分解穩定度
        ...
        # 關鍵修正：計算 W 與 W 作用後的對應均值平移項 b_align，達成形狀與幾何中心點雙重對齊
        W_align = cov_tgt_sqrt_inv @ coral_cov_src_sqrt  # [hidden, hidden]
        b_align = coral_mu_src - (mu_tgt @ W_align)       # [hidden]
    ...
    with torch.no_grad():
        for segs, y, _ in loader:
            ...
            # 先跑常規的 Z-Score 並進入 GRU 網路層提取原始特徵
            segs_norm = (segs - global_mu) / global_std
            logits_base, feat_base = model(segs_norm)   # 同時取得出廠預測與高維特徵
            ...
            # 在 [Batch, hidden] 特徵層面套用全域骨架旋轉對齊與中心平移
            feat_aligned = feat_base @ W_align + b_align
            logits_align = model.fc(feat_aligned)       # 對齊後特徵直接送入 FC 分類器
