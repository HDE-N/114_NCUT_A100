...
# 保留參數槽，確保相容網格搜尋大腳本
parser.add_argument("--lambda_dann", type=float, default=1.0, help="保留參數槽")
parser.add_argument("--target_version", type=str, default='46/special_data', help="目標域資料夾路徑")
...
def main():
    ...
    align_weights_path = os.path.join(model_dir, "coral_weights.pth")
    ...
    print(f"Testing Center Alignment Model (Evaluation Unit: Segment): {run_name}")
    ...
    # 自動加載解算出來的平移參數 W 與 b
    if os.path.exists(align_weights_path):
        align_weights = torch.load(align_weights_path, map_location=device)
        W_align = align_weights['W'].to(device)
        b_align = align_weights['b'].to(device)
        print("-> 基準中心點平移參數載入成功！(推論時將自動執行特徵基準對齊)")
        has_alignment = True
    else:
        print("[Warning] 找不到 alignment weights (coral_weights.pth)，將使用原始資料直接進行推論。")
        has_alignment = False
    ...
    with torch.no_grad():
        for segs, y, _ in loader:
            ...
            # 測試集基準平移修正：將新使用者特徵平移回 Source 的基準線上
            if has_alignment:
                segs = segs @ W_align + b_align
    ...
    # 2. 透過 classification_report 完整抓取各類別的 P, R, F1
