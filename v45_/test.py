...
# =============================================================================
# 關鍵修正：適配新版 DANN + Global Average Pooling 骨架網路模型
# =============================================================================
class DANNGRUModel(nn.Module):
    def __init__(self, input_dim=1, hidden=128, layers=2, num_classes=3):
        ...
        # 必須保留領域鑑別器宣告，以防 torch.load 出現模型權重鍵值不匹配錯誤
    ...
    def forward(self, x, alpha=1.0):
        ...
        # out_seq Shape: [batch_size, seq_len, hidden]
        out_seq, _ = self.gru(x)
        
        # 核心對齊：全時段平均池化 (Global Average Pooling)
        feature = torch.mean(out_seq, dim=1) 
        class_output = self.fc(feature)
        
        # 推論評估期不需要執行梯度反轉與領域預測，直接回傳 None 節省晶片算力
        return class_output, feature, None
...
def main():
    ...
    if len(calib_ds) == 0:
        ...
        # 1. 透過目前 DANN 模型提取校正資料之特徵 (核心修正：改為三輸出解包)
        print("-> 正在經由 GRU 提取邊緣端 Target 領域總體特徵空間...")
        with torch.no_grad():
            base_calib_logits, calib_feat, _ = model(calib_segs_norm)
    ...
    with torch.no_grad():
        for segs, y, _ in loader:
            ...
            # 先跑常規的 Z-Score 並進入 GRU 網路層提取經 DANN 抗個體干擾後的特徵
            segs_norm = (segs - global_mu) / global_std
            logits_base, feat_base, _ = model(segs_norm)   # 核心修正：改為三輸出解包
