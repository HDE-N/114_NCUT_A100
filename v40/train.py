...
def main():
    ...
    # 將所有訓練數據的形狀攤平為 [Total_Samples * 960, Channels] 來算各通道的均值與標準差
    ...
    global_std = all_train_flat.std(dim=0) + 1e-8 # 加上 epsilon 避免除以零
    ...
    # ===== 5. 核心：提取並打包邊緣端部署所需的所有參數 =====
    print("\n====== 正在提取 Source 領域 (notTired) 的黃金基準統計量 ======")
    if len(train_n) > 0:
        # 注意：此時的 train_n.data 已經是經過 Z-Score 正規化過的數據了
        src_notTired_flat = train_n.data.view(-1, input_dim).to(device)
        
        coral_mu_src = src_notTired_flat.mean(dim=0)
        src_centered = src_notTired_flat - coral_mu_src
        coral_cov_src = (src_centered.T @ src_centered) / (src_notTired_flat.size(0) - 1) + torch.eye(input_dim).to(device) * 1e-5
        
        Us, Ss, Vs = torch.linalg.svd(coral_cov_src)
        # 加入 clamp 確保奇異值大於 0，防止 sqrt 產生 NaN
        coral_cov_src_sqrt = Us @ torch.diag(torch.sqrt(torch.clamp(Ss, min=1e-6))) @ Vs
        
        # 將全域正規化參數與 CORAL 參數打包成一個部署檔
