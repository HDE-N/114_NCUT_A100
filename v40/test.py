...
def main():
    ...
    # =========================================================
    # 階段 2：邊緣端個人化校正 (包含 Base 與 Coral 的閾值計算)
    # =========================================================
    print("\n====== 正在執行邊緣端個人化校正 ======")
    ...
    base_active_th = args.ceiling # 新增：給無輸入自適應的專屬閾值

    fallback_floor = 0.20 

    if len(calib_ds) == 0:
        print(f"[Warning] 找不到校正數據，放棄 CORAL 與動態閾值，使用基礎值 (門檻={active_th})")
        W_align = torch.eye(input_dim).to(device)
        b_align = torch.zeros(input_dim).to(device)
    else:
        ...
        calib_flat = calib_segs_norm.view(-1, input_dim) 
        
        # 1. 特徵對齊矩陣計算 (CORAL)
        mu_tgt = calib_flat.mean(dim=0)
        tgt_centered = calib_flat - mu_tgt
        cov_tgt = (tgt_centered.T @ tgt_centered) / (calib_flat.size(0) - 1) + torch.eye(input_dim).to(device) * 1e-5
        ...
        print(f"-> 特徵對齊矩陣計算完成 ({len(calib_ds)} 片段)")
        
        calib_segs_aligned = calib_segs_norm @ W_align + b_align
        
        with torch.no_grad():
            # 2. 計算 Base 版的動態閾值 (無特徵對齊，僅輸入標準化資料)
            base_calib_logits = model(calib_segs_norm)
            ...
            # 3. 計算 CORAL 版的動態閾值 (輸入特徵對齊後資料)
            calib_logits = model(calib_segs_aligned)
    ...
    with torch.no_grad():
        for segs, y, _ in loader:
            ...
            # --- 狀態 3 & 4: 特徵對齊後 (CORAL & Adapt 共享機率分佈) ---
            segs_aligned = segs_norm @ W_align + b_align
            logits_align = model(segs_aligned)
    ...
    # =========================================================
    # 指標評估與 CSV 紀錄設計 (加入 prefix 以便單列全紀錄)
    # =========================================================
    classes = [0, 1, 2]
    ...
    # 信心值圖 (注意：BASE 與 BASE_ADAPT 共用 all_probs_base，CORAL 與 ADAPT 共用 all_probs_align)
